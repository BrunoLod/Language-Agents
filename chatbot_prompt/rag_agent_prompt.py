...
template = """\
    ...
    - Responda a `Query` utilizando os trechos recuperados de `Context`
    - Caso não consiga responder à mensagem ou os trechos recuperados não sejam semanticamente relevantes, diga que não conseguirá responder à mensagem do usuário, mas seja proativa, trazendo uma resposta suposta do que pode a partir do seu pré-conhecimento compreender, aconselhando ao mesmo tempo o usuário a pesquisar em locais especializados. 
